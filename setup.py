...
if (os.path.exists(PROCESSING_DIR) and not debug_skip_downloading):
...
# Install processing
# CHANGE: Processing now compresses linux downloads with ZIP, not TGZ.
if (not debug_skip_downloading):
    ...
    try:
        if (os.name == "nt"):
            ...
            download_file(PROCESSING_DOWNLOAD_URL_LINUX, install_path+'processing.zip')
        else:
    ...
    # No need to check system since all downloads are now TGZ
    try:
        # if (os.name == "nt"):
        ...
        error("Extract error, perhaps file is corrupted?")
        exit(1)
...
if (os.path.exists("processing.tgz")):
    os.remove(install_path+"processing.tgz")
...
print("Patching core...")
if (os.path.exists(PROCESSING_DIR+"app/resources/core")):
    shutil.rmtree(PROCESSING_DIR+"app/resources/core")
else:
...
# Copy core patch
shutil.copytree(CORE_DIR, PROCESSING_DIR+"app/resources/core")
...
if (os.path.exists(PROCESSING_DIR+"app/resources/modes/java/application")):
    copy_from(ICONS_DIR, PROCESSING_DIR+"app/resources/modes/java/application")
else:
    print(color.GOLD+"Warning: "+PROCESSING_DIR+"app/resources/modes/java/application not found. This may indicate a corrupted install."+color.NONE)


# Stage 5:
# Theme and thumb install
print("Installing theme...")
if (os.path.exists(PROCESSING_DIR+"app/resources/lib/theme")):
    # Copy theme
    shutil.copyfile(THEME_DIR+"/theme.txt", PROCESSING_DIR+"app/resources/lib/theme/theme.txt")
    # Copy default theme
    if (os.path.exists(PROCESSING_DIR+"app/resources/lib/theme/Alloys/imilac.txt")):
        shutil.copyfile(THEME_DIR+"/theme.txt", PROCESSING_DIR+"app/resources/lib/theme/Alloys/imilac.txt")
    # Eh can't be bothered with warnings for this part.
else:
    print(color.GOLD+"Warning: "+PROCESSING_DIR+"app/resources/lib/theme not found. This may indicate a corrupted install."+color.NONE)
